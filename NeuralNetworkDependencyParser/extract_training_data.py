...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        for i in range (3):
            if (i >= len (state.stack)):
                arr [i] = self.word_vocab["<NULL>"]
            else:
                if (state.stack[-(i+1)] == 0):
                    arr[i] = self.word_vocab["<ROOT>"]
                elif (pos [state.stack[-(i+1)]].lower() == "cd"):
                    arr[i] = self.word_vocab["<CD>"]
                elif (pos [state.stack[-(i+1)]].lower() == "nnp"):
                    arr[i] = self.word_vocab["<NNP>"]
                elif (words [state.stack[-(i+1)]].lower() not in self.word_vocab):
                    arr[i] = self.word_vocab["<UNK>"]
                else:
                    arr [i] = self.word_vocab [words [state.stack[-(i+1)]].lower()]

        for i in range(3):
            if (i >= len(state.buffer)):
                arr[i + 3] = self.word_vocab["<NULL>"]
            else:
                if (state.buffer[-(i + 1)] == 0):
                    arr[i + 3] = self.word_vocab["<ROOT>"]
                elif (pos[state.buffer[-(i + 1)]].lower() == "cd"):
                    arr[i + 3] = self.word_vocab["<CD>"]
                elif (pos[state.buffer[-(i + 1)]].lower() == "nnp"):
                    arr[i + 3] = self.word_vocab["<NNP>"]
                elif (words[state.buffer[-(i + 1)]].lower() not in self.word_vocab):
                    arr[i + 3] = self.word_vocab["<UNK>"]
                else:
                    arr[i + 3] = self.word_vocab[words[state.buffer[-(i + 1)]].lower()]

        '''for i in range (1, len(state.stack) + 1):
            #arr [i - 1] = self.word_vocab[state.stack [-i]]
            arr[i - 1] = state.stack[-i]
        for i in range (len(state.stack), 3):
            arr[i] = self.word_vocab["<NULL>"]

        for i in range(1, len(state.buffer) + 1):
            #arr[i + 2] = self.word_vocab[state.buffer[-i]]
            arr[i + 2] = state.buffer[-i]
    ...
    def get_output_representation(self, output_pair):  
        # TODO: Write this method for Part 2
        arr = np.zeros (91)
        if (output_pair [1] == None):
            arr [0] = 1
        elif (output_pair[0] == "left_arc"):
            arr[1 + dep_relations.index(output_pair[1])] = 1
        elif (output_pair[0] == "right_arc"):
            arr[46 + dep_relations.index(output_pair[1])] = 1
        '''elif (output_pair[0] == "left_arc"):
            arr[1 + 2 * dep_relations.index(output_pair[1])] = 1
        elif (output_pair[0] == "right_arc"):
            arr[2 + 2 * dep_relations.index(output_pair[1])] = 1'''
        return arr
